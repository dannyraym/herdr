...
Stacked panes share one region inside the pane's layout slot: the expanded
pane keeps the space and every other member collapses to a one-line title
bar, leaving surrounding splits untouched. Focusing a collapsed pane expands
it. Directional splits treat a stack as a single pane and split its whole
region, while `pane.split` with `stacked: true` adds the new pane to the
target's stack instead, creating one if needed (this is what the `stack_pane`
keybind and the pane context menu use). `pane.stack` merges the target pane
with its layout sibling into one stack — an API/CLI operation for gathering
existing panes, with no default keybinding.
`pane.unstack` peels one edge pane off the target's stack, as in zellij: the
top pane breaks out upward when the expanded pane sits at the bottom of the
stack, otherwise the bottom pane breaks out downward; a stack left with one
pane dissolves.

```json
{"id":"req_stack","method":"pane.stack","params":{"pane_id":"w1:p1"}}
{"id":"req_unstack","method":"pane.unstack","params":{"pane_id":"w1:p1"}}
{"id":"req_split_stacked","method":"pane.split","params":{"pane_id":"w1:p1","direction":"down","stacked":true}}
```

Omitting `pane_id` targets the server's active focused pane. The responses are
`type: "pane_stack"` and `type: "pane_unstack"` with `changed`,
`stack_changed`, `focus_changed`, optional `reason`, `pane_id`,
`focused_pane_id`, `stacked`, and `layout`. Reason values are `single_pane`,
`no_sibling`, and `not_in_stack`. Layout trees returned by `pane.layout` and
`layout.export` describe a stack as a `stack` node with `panes` and an
`expanded` index, and `layout.apply` accepts the same node shape.
